                    ...
                    <tbody>
                        <?php foreach ($latest_bets as $bet): ?>
                            <?php
                                $gameType = strtolower($bet['game_type']);
                                $gameAsset = $game_assets[$gameType] ?? null;
                                $gameName = $gameAsset['name'] ?? $bet['game_name'];
                                $amount = (float)$bet['wager_amount'];
                                $payout = (float)$bet['payout_amount'];
                                $net = (float)$bet['net_result'];
                                $multiplier = $amount > 0 ? $payout / $amount : 0;
                                $isWin = $net > 0;
                            ?>
                            <tr data-bet-row data-is-mine="<?php echo $user_id && (int)$bet['user_id'] === $user_id ? 'true' : 'false'; ?>">
                                <td data-label="Game">
                                ...
                                <td data-label="User"><?php echo htmlspecialchars(mask_username($bet['username']), ENT_QUOTES, 'UTF-8'); ?></td>
